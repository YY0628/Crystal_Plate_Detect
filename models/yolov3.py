...
from models.common import Conv, ResBlock
...
class Darknet53(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer0 = nn.Sequential(OrderedDict([("Conv", Conv(1, 32, 3, 1, act="leaky"))]))        # 此出第一个参数，表示了图像的颜色通道数   这里是 1 表示单通道
...
if __name__ == '__main__':
    ...
    x = torch.ones((4, 1, 608, 608)).to(device)
    model = YOLOv3(class_num=1, image_size=608).to(device)
    model.eval()
    out = model(x)
